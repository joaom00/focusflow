...
export const useTasksQuery = () => {
  return useQuery<Task[]>(
    ...
    { staleTime: THREE_MINUTES, retry: 0 }
  )
}
...
export const useCreateTaskMutation = () => {
  ...
  return useMutation(
    async ({ id, content, position }: CreateTaskPayload) => {
      const { data } = await api.post('tasks', { id, content, position })
      return data
    },
    {
...
export const useUpdateTaskMutation = () => {
  ...
  return useMutation(
    async ({ id, content }: UpdateTaskPayload) => {
